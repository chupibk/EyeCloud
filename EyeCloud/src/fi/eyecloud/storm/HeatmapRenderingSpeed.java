...
public class HeatmapRenderingSpeed {
	...
	public static class ReadData extends BaseBasicBolt {
        ...
        public void execute(Tuple tuple, BasicOutputCollector collector) {
        	...
        	float value[] = new float[Integer.parseInt(data[1])*Constants.PARAMETER_NUMBER_HEATMAP];
        	ReadTextFile readFile = new ReadTextFile(data[0]);
        	int duration = Constants.THOUSAND/Constants.SAMPLE_RATE;
        	for (int i=0; i < value.length/Constants.PARAMETER_NUMBER_HEATMAP; i++){
        		readFile.readNextLine();
        		value[i*Constants.PARAMETER_NUMBER_HEATMAP] = Float.parseFloat(readFile.getField(Constants.GazePointX));
        		value[i*Constants.PARAMETER_NUMBER_HEATMAP + 1] = Float.parseFloat(readFile.getField(Constants.GazePointY));
        		value[i*Constants.PARAMETER_NUMBER_HEATMAP + 2] = duration;
        	}
	...
	public static class AggregatorData extends BaseBatchBolt {
        ...
        public void finishBatch() {
			...
			_collector.emit(new Values(_id, "Ok: " + time + " - " + color.getTimeMax()));
        }
    ...
    public static void main(String[] args) throws Exception {
        ...
        if(args==null || args.length==0) {
            ...
            String send = "data/heatmap/100000.txt" + Constants.PARAMETER_SPLIT + "100000" + Constants.PARAMETER_SPLIT;
